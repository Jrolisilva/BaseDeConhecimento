    ...
    <b-form>
      ...
      <b-form-group label="Nome" label-for="article-name">
        <b-form-input id="article-name" type="text" v-model="article.name" 
           required :readonly="mode === 'remove'" placeholder="Nome do Artigo"/>
      </b-form-group>
      <b-form-group label="Descrição" label-for="article-description">
        <b-form-input id="article-description" type="text"  v-model="article.description"
          required  :readonly="mode === 'remove'" placeholder="Descrição do Artigo"/>
      </b-form-group>
      <b-form-group label="Imagem (URL)" label-for="article-imageUrl">
        <b-form-input id="article-imageUrl" type="text" v-model="article.imageUrl"
          required :readonly="mode === 'remove'" placeholder="Informe a Url da Imagem"/>
      </b-form-group>
...
export default {
  ...
  methods: {
    loadArticles() {
      ...
      axios.gt(url).then(res => {
          this.articles = res.data.data
          this.count = res.data.count
          this.limtit = res.data.limit
      }) 
    },
    reset() {
      this.mode = "save";
    ...
    loadArticle(article, mode = "save") {
      ...
      axios.get(`${baseApiUrl}/arcticles/${article.id}/`)
